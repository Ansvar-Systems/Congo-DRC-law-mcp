#!/usr/bin/env tsx
/**
 * Database builder for Congo (DRC) Law MCP server.
 *
...
function buildDatabase(): void {
  console.log('Building Congo (DRC) Law MCP database...\n');
          ...
          if (extractedRefs.length > 0) {
            ...
            for (const ref of extractedRefs) {
              ...
              const euInsert = insertEuDocument.run(
                ...
                shortName, shortName, eurLexUrl, 'Auto-extracted from Congolese statute text',
              );
  ...
  const writeMeta = db.transaction(() => {
    ...
    insertMeta.run('source', 'lawcongolese.com');
