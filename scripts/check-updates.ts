#!/usr/bin/env tsx
/**
 * Congolese Law MCP — Data Freshness Checker
 *
...
const PORTAL_URL = 'http://lawcongolese.com';
const PORTAL_NAME = 'Congo (DRC) Law';
...
async function checkPortal(url: string): Promise<boolean> {
  try {
    ...
    const res = await fetch(url, {
      ...
      headers: { 'User-Agent': '@ansvar/congolese-law-mcp/1.0 (data-freshness-check)' },
    });
...
async function main(): Promise<void> {
  console.log('Congolese Law MCP — Data Freshness Check');
