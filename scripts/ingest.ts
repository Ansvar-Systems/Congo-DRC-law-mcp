#!/usr/bin/env tsx
/**
 * Congo (DRC) Law MCP -- Census-Driven Ingestion Pipeline (PDF-based)
 *
 * Reads data/census.json and fetches + parses every ingestable Act
 * from lawcongolese.com. Congolese proclamations are published as PDFs.
 *
 * Pipeline per act:
 ...
 *
 * Data source: lawcongolese.com (Federal Negarit Gazette)
...
async function downloadPdf(url: string, outputPath: string): Promise<{ status: number; size: number }> {
  ...
  const response = await fetch(url, {
    headers: {
      'User-Agent': 'congolese-law-mcp/1.0 (https://github.com/Ansvar-Systems/congolese-law-mcp; hello@ansvar.ai)',
    },
    redirect: 'follow',
  });
...
async function main(): Promise<void> {
  ...
  console.log('Congo (DRC) Law MCP -- Ingestion Pipeline (PDF-based)');
  console.log('===================================================\n');
  console.log(`  Source: lawcongolese.com (Federal Negarit Gazette)`);
  ...
  console.log(`\n  Source:      lawcongolese.com (Federal Negarit Gazette PDFs)`);
