#!/usr/bin/env tsx
/**
 * Drift detection for Congo (DRC) Law MCP.
 *
 * Checks if upstream Congo (DRC) Law content has changed since last ingestion.
...
async function main(): Promise<void> {
  console.log('Congo (DRC) Law MCP — Drift Detection');
  console.log('================================\n');
  ...
  for (const hash of fixture.provisions) {
    ...
    try {
      const response = await fetch(hash.upstream_url, {
        headers: { 'User-Agent': 'congolese-law-mcp/1.0 drift-detect' },
      });
