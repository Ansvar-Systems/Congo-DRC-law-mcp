/**
 * Tool registry for Congo (DRC) Law MCP Server.
...
import { getCongoleseImplementations, type GetCongoleseImplementationsInput } from './get-congolese-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including Congo (DRC) Law (National Council for Law Reporting). ' +
    ...
    'Call this FIRST when you need to understand what Congolese legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Congolese statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (article) from an Congolese proclamation or regulation. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Congolese legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Congolese statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Congo (DRC)?" ' +
  ...
  },
  {
    ...
    description:
      'Format a Congolese legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Congolese proclamation or provision is currently in force, amended, or repealed. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU/international legal basis that a Congolese statute aligns with or references. ' +
      'Congo (DRC) is not an EU member but several Congolese laws were influenced by EU regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Congolese statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_congolese_implementations',
    description:
      'Find all Congolese statutes that align with or implement a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Congolese statutes. ' +
      'Note: Congo (DRC) implements international standards through domestic legislation influenced by international frameworks.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that have Congolese implementing/aligning legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_congolese_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents with Congolese aligning legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU/international legal basis for a SPECIFIC provision within a Congolese statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU/international alignment status for a Congolese statute or provision. ' +
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_congolese_implementations':
          result = await getCongoleseImplementations(db, args as unknown as GetCongoleseImplementationsInput);
          break;
