/**
 * search_eu_implementations — Search EU directives/regulations that have Congolese implementing legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_congolese_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  congolese_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as congolese_statute_count
  ...
  if (input.has_congolese_implementation) {
    sql += ' HAVING congolese_statute_count > 0';
  }
