/**
 * get_congolese_implementations — Find Congolese statutes implementing a specific EU directive/regulation
...
export interface GetCongoleseImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface CongoleseImplementationResult {
  document_id: string;
...
export async function getCongoleseImplementations(
  db: InstanceType<typeof Database>,
  input: GetCongoleseImplementationsInput,
): Promise<ToolResponse<CongoleseImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as CongoleseImplementationResult[];
