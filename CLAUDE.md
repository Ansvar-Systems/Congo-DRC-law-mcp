# Congo (DRC) Law MCP Server — Developer Guide
...
Congo (DRC) Law MCP server providing Congolese legislation search via Model Context Protocol. Strategy A deployment (Vercel, bundled SQLite DB). Covers data protection, cybercrimes, ICT, companies, consumer protection, and other key Acts.
...
- Congo (DRC) uses "Section N" for Acts and "Article N" for the Constitution
...
1. `scripts/ingest.ts` -> fetches from Congo (DRC) Law -> JSON seed files in `data/seed/`
...
- **Congo (DRC) Law** (congoleselaw.org) -- National Council for Law Reporting
- **License:** Government Open Data
- **Languages:** English (en) is the primary legal language; Swahili (sw) for some documents
- **Coverage:** All Acts of Parliament, subsidiary legislation, Constitution of Congo (DRC) 2010, selected case law

## Congo (DRC)-Specific Notes

- Congo (DRC) uses a common law legal system inherited from British colonial administration
- The Constitution of Congo (DRC) 2010 is the supreme law (Article 2)
...
- For the Constitution: "Article N, Constitution of Congo (DRC) 2010"
...
- npm package: `@ansvar/congolese-law-mcp` with bin entry for stdio
